...
from gigui.typedefs import (
    OID, SHA, Author, FileStr, Rev, UnixTimestamp,
    CommitGroupsDict, LineCountDict, SHAToAuthorDict
)
...
class RepoBase:
    ...
    def __init__(self, ini_repo: IniRepo):
        ...
        self.head_commit: Optional[GitCommit] = None
        self.head_oid: Optional[OID] = None
        self.head_sha: Optional[SHA] = None
    ...
    def _matches_ex_file(self, fstr: FileStr) -> bool:
        ...
        """
        return any(
            fnmatchcase(fstr.lower(), pattern.lower()) 
        ...
        def reduce_commits():
            ...
            while fstrs:
                ...
                for fstr2 in fstrs:
                    ...
                    i = -1
                    while (commit_groups2 and 
                           abs(i) <= len(commit_groups1) and
                           commit_groups1[i] == commit_groups2[-1]):
                        commit_groups2.pop()
                        i -= 1
                    ...
                    for future in as_completed(futures):
                        try:
                            ...
                            if self.args.verbosity == 0:
                                ...
                                logger.info(
                                    f"{prefix}log {i} of {i_max}: "
                                    f"{self.name}: {fstr}" if self.args.multicore 
                                    else f"{fstr}"
                                )
