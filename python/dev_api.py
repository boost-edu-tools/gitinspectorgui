...
def main():
    ...
    try:
        # Ensure we can find the gigui module regardless of working directory
        import sys
        from pathlib import Path
        ...
        for i in range(10):
            potential_path = current_path / "python"
            print(f"  Checking: {potential_path} (exists: {potential_path.exists()})", file=sys.stderr)
            if potential_path.exists():
                gigui_path = potential_path / "gigui"
                print(f"    gigui at: {gigui_path} (exists: {gigui_path.exists()})", file=sys.stderr)
            current_path = current_path.parent
